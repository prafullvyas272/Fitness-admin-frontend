...
// Normalizes the API's `user` field to `trainer` so the rest of the UI stays unchanged
const normalize = (r) => ({ ...r, trainer: r.user });
...
const StatusBadge = ({ status }) => {
  const styles = {
    ...
    PENDING:  { bg: "rgba(248,227,150,0.12)",  border: "rgba(248,227,150,0.3)",  color: "#f8e396" },
  };
...
export default function DeleteRequests() {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/api/account-deletion-requests/trainers`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      if (res.ok && data.success) {
        setRequests((data.data || []).map(normalize));
      } else {
  ...
  const handleAction = async (id, status) => {
    if (status === "APPROVED") {
      if (!window.confirm("Approving this will permanently delete the trainer's account. Continue?")) return;
    }
    setActionLoading(id + status);
    try {
      const token = localStorage.getItem("adminToken");
      const res = await fetch(`${BASE_URL}/api/account-deletion-requests/${id}/status`, {
        method: "PATCH",
      ...
      const updated = normalize(data.data.updatedRequest);
  ...
  const filtered = requests.filter((r) => {
    const name = `${r.trainer?.firstName} ${r.trainer?.lastName}`.toLowerCase();
    const matchSearch =
  ...
  return (
    <div style={{ background: G.bg, minHeight: "100vh", padding: 24 }}>
      <style>{`
        ...
        .dr-thead th { background: #111111 !important; color: rgba(248,227,150,0.6) !important; border-bottom: 1px solid ${G.divider} !important; font-size: 10px !important; letter-spacing: 1.2px !important; padding: 12px 16px !important; font-weight: 700 !important; }
        .dr-tbody tr td { background: ${G.card} !important; color: #cccccc !important; border-bottom: 1px solid #141414 !important; padding: 14px 16px !important; vertical-align: middle; font-size: 12px !important; font-weight: 600 !important; line-height: 1.4 !important; font-family: Montserrat, Arial, sans-serif !important; }
        .dr-tbody tr:hover td { background: #111111 !important; }
        .dr-inp { background: ${G.input} !important; border: 1px solid ${G.divider} !important; color: #cccccc !important; border-radius: 7px; outline: none; padding: 10px 13px; font-size: 12.5px; width: 100%; max-width: 320px; }
        .dr-inp::placeholder { color: #2a2a2a !important; }
        .dr-inp:focus { box-shadow: none !important; border-color: rgba(248,227,150,0.25) !important; }
      ...
      <div style={{ marginBottom: 24 }}>
        <h3 style={{ color: G.text, fontWeight: 700, margin: 0 }}>Delete Requests</h3>
        <small style={{ color: G.muted }}>Review and process trainer account deletion requests</small>
      </div>
      ...
      <div style={{ background: G.card, border: `1px solid ${G.divider}`, borderRadius: 14, padding: 24 }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {filterBtns.map((s) => {
              ...
              const colors = {
                ...
                PENDING:  { active: "#f8e396", text: "#000" },
        ...
        <Table responsive className="align-middle mb-0" style={{ minWidth: 800 }}>
          ...
          <tbody className="dr-tbody">
            {loading ? (
              <tr>
                <td colSpan={6} style={{ textAlign: "center", padding: "40px 0", color: G.muted }}>
              ...
              paginated.map((req, idx) => (
                <tr key={req.id}>
                  ...
                  <td style={{ maxWidth: 220, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", color: G.muted, fontSize: 13 }}>
                    {req.reason || "—"}
                  </td>
                  <td style={{ fontSize: 13, color: G.muted, whiteSpace: "nowrap" }}>
                    {(req.createdAt || req.requestedAt) ? new Date(req.createdAt || req.requestedAt).toLocaleDateString("en-US", { month: "2-digit", day: "2-digit", year: "numeric" }) : "—"}
                  </td>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered size="lg" className="modal-gold">
        <Modal.Header closeButton>
          <Modal.Title>Delete Request Details</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selected && (
            <Row>
              <Col md={12}>
                <div style={{ color: G.muted, fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 12 }}>Trainer</div>
              ...
              <Col xs={12} style={{ paddingTop: 16, borderTop: `1px solid ${G.divider}` }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  ...
                  {selected.reason && (
                    <div><span style={{ color: G.muted, fontSize: 13 }}>Reason: </span><span style={{ color: G.text }}>{selected.reason}</span></div>
                  )}
                  {(selected.createdAt || selected.requestedAt) && (
                    <div><span style={{ color: G.muted, fontSize: 13 }}>Requested On: </span><span style={{ color: G.text }}>{new Date(selected.createdAt || selected.requestedAt).toLocaleString("en-IN")}</span></div>
                  )}
                </div>
              </Col>
            </Row>
