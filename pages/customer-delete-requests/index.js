...
// Normalizes the API's `user` field to `customer` so the rest of the UI stays unchanged
const normalize = (r) => ({ ...r, customer: r.user });
...
export default function CustomerDeleteRequests() {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/api/account-deletion-requests/customers`, {
  ...
  const handleAction = async (id, status) => {
    if (status === "APPROVED") {
      if (!window.confirm("Approving this will permanently delete the customer's account. Continue?")) return;
    }
  ...
  return (
    <div style={{ background: G.bg, minHeight: "100vh", padding: 24 }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <small style={{ color: G.muted }}>Review and process customer account deletion requests</small>
      </div>
